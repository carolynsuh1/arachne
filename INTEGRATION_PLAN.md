...
- **Operational requirement:** voice additionally requires the same non-empty `INTERNAL_API_KEY` in root `.env` and `backend/.env`. Missing-key errors are non-reconnectable; an offline backend remains a recoverable UI error. After pulling, run `npx prisma db push` so local SQLite has `backendPersonId` / `backendGoalId`; if `/map` is missing the feature toolbar or Next 500s with `Cannot find module './570.js'`, delete `.next` and restart the web app.
