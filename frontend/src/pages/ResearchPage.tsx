...
export function ResearchPage({target:initialTarget,goal,viewerId,viewerName,onOpenProfiles,onDebrief}:{target?:ResearchTarget;goal?:string;viewerId?:string;viewerName?:string;onOpenProfiles:()=>void;onDebrief?:(person:ResearchTarget)=>void}) {
 ...
 useEffect(()=>{
  let active=true
  setName(target?.name??"");setSchool("");setProfile("");setError("");setResult(null)
  setLoadingBrief(!!target)
  if(target)latestResearch(target.id)
   .then(r=>{if(active)setResult(r)})
   .catch(e=>{if(active)setError(e instanceof Error?e.message:"Could not load saved research. Please retry.")})
   .finally(()=>{if(active)setLoadingBrief(false)})
  return()=>{active=false}
 },[target?.id])
 async function run(url?:string) {
  if(busy||loadingBrief)return
 ...
 return <div className="mx-auto max-w-4xl space-y-6">
  ...
  <fieldset disabled={busy}><PersonDataPanel target={target} onSelect={setTarget} name={name} result={result} onBrief={setResult} loadingBrief={loadingBrief}/></fieldset>
  <form onSubmit={e=>{e.preventDefault();void run(profile||undefined)}} className="rounded-xl border border-stone-300 bg-stone-50 p-5">
   <fieldset disabled={busy||loadingBrief} className="space-y-4 disabled:opacity-60"><div className="grid gap-4 md:grid-cols-2">
  ...
  <p role="status" className="font-sans text-sm text-stone-600">{loadingBrief?"Loading saved research...":questionBusy?"Writing and reviewing questions against both profiles...":busy?"Checking sources. New profile scrapes can take up to 90 seconds.":result?.status==='ready'?"Brief ready. Profile claims are not independently verified.":""}</p>
  ...
  {result&&<div className="space-y-5">
   ...
   {result.status==='ready'&&<><h2 className="text-2xl">{result.person}</h2>{result.brief_id&&<p className="font-sans text-xs text-stone-500">Saved brief{target?" for this person":""}</p>}
    {result.brief_id&&<button disabled={busy||loadingBrief||!viewerId} className="rounded-full border border-stone-400 px-4 py-2 disabled:opacity-50" onClick={()=>{if(busy||loadingBrief||!result.brief_id||!viewerId)return;setBusy(true);setQuestionBusy(true);setError("");void personalizeQuestions(result.brief_id,viewerId,meeting).then(setResult).catch(e=>setError(e.message)).finally(()=>{setBusy(false);setQuestionBusy(false)})}}>Regenerate questions using my saved details</button>}
