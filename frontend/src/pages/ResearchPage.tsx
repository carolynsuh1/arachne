...
import { latestResearch, researchPerson } from "../api"
...
export function ResearchPage({target,goal}:{target?:ResearchTarget;goal?:string}) {
 ...
 useEffect(()=>{let active=true;if(target)latestResearch(target.id).then(r=>{if(active)setResult(r)}).catch(()=>{});return()=>{active=false}},[target])
 async function run(url?:string) {
  if(busy)return
  setBusy(true);setError("");setResult(null)
  const input:ResearchInput={name,affiliation:school,goal:meeting,...(url?{profileUrl:url}:{}),...(target?{person_id:target.id}:{})}
 ...
 return <div className="mx-auto max-w-4xl space-y-6">
  ...
  <form onSubmit={e=>{e.preventDefault();void run(profile||undefined)}} className="rounded-xl border border-stone-300 bg-stone-50 p-5">
   <fieldset disabled={busy} className="space-y-4 disabled:opacity-60"><div className="grid gap-4 md:grid-cols-2">
  ...
  <p role="status" className="font-sans text-sm text-stone-600">{busy?"Checking sources. New profile scrapes can take up to 90 seconds.":result?.status==='ready'?"Brief ready. Profile claims are not independently verified.":""}</p>
  ...
  {result&&<div className="space-y-5">
   ...
   {result.status==='ready'&&<><h2 className="text-2xl">{result.person}</h2>{result.brief_id&&<p className="font-sans text-xs text-stone-500">Saved brief{target?" for this person":""}</p>}
    {result.questions?.length?<section className="rounded-xl bg-stone-900 p-5 text-white"><h3 className="text-xl">Conversation starters</h3><ol className="mt-3 list-decimal space-y-3 pl-5">{result.questions.map((q,i)=><li key={i}>{q.text}</li>)}</ol></section>:null}
