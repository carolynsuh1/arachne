...
import {cloudProfiles} from './cloud-profiles.js';
const here=dirname(fileURLToPath(import.meta.url));
export function buildWebServer(research,{origin='http://127.0.0.1:8790',hourlyLimit=100,concurrentLimit=2,now=Date.now}={}){
 ...
 return http.createServer(async(req,res)=>{
  ...
  if(req.method!=='POST'||req.url!=='/api/research')return send(404,{error:'Not found'});
  ...
  let body='';try{for await(const chunk of req){body+=chunk;if(Buffer.byteLength(body)>8192)return send(413,{error:'Request too large.'});}}catch{return send(400,{error:'Request interrupted.'});}
  ...
  requests++;active++;
  try{return send(200,await research(input));}
